...
class PanelInput(wx.Panel):
    ...
    def click_export(self, event):
        try:
            ...
            self._logging(e, 'PermissionError : エクスポート失敗。ファイルアクセス権限に問題があります。処理を中断します。')

        except UnicodeDecodeError as e:
            self._logging(e, 'UnicodeDecodeError : エクスポート失敗。ファイルエンコードに問題があります。処理を中断します。')
    ...
    def thread_processffmpeg(self, newsourcelist, minecraft_edition):
        ...
        for newsource, ogg in newsourcelist:
            ...
            if category == 'bgm' or category == 'se':
                ...
                # JEでは、pigstep.oggとなっており、BEでは、pigstep_master.oggとなっている。
