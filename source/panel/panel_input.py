...
class PanelInput(wx.Panel):
    ...
    def click_export(self, event):
        ...
        flag_zip = self.checkbox_zip_compression.GetValue()  # 20230210 圧縮指定True zip圧縮処理前に再度取得している。
        ...
        if edition == 'BE' and flag_zip == True:
            ...
            resourcepack = os.path.join(savepath, name)  # 20230210 BE JE どちらも拡張子無しのフォルダ名
        ...
        # except:
        # self.show_message_error('エクスポートに問題が発生しました。')
    ...
    def get_desktoppath(self):  # 20230210
    ...
    def _newsource_in_blankpack(self, minecraft_edision):
        ...
        thread_processffmpeg = threading.Thread(target=self.thread_processffmpeg(newsourcelist, minecraft_edision))  # progressDialog 非表示のため、第三引数省略。
    ...
    def get_distdir(self, ogg, minecraft_edition):
        ...
        if minecraft_edition == 'BE':
            ...
            elif name in bgmlist_nether:  # 新曲はBEと階層が違う　sounds.json参照
                if name == 'chrysopoeia':
    ...
    def set_packdata(self, packdata):
        ...
        self.bundle = packdata[7]  # 20230209
        self.label_bundle.SetLabel('bundle : ' + str(len(self.bundle)))  # 20230209
