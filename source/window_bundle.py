...
class WindowBundle(wx.Dialog):
    ...
    def __init__(self, parent, bundle):
        wx.Frame.__init__(self, parent, title="select bundle folder", size=(
        ...
        base_toolbar = wx.Panel(self, pos=(0, 0), size=(
            self.WIDTH, self.TOOLBAR_HEIGHT))
        base_toolbar.SetBackgroundColour('#969696')
        self.toolbar = wx.Panel(base_toolbar, pos=(
            0, 0), size=(self.WIDTH, self.TOOLBAR_HEIGHT-1))
        self.toolbar.SetBackgroundColour(wx.WHITE)

        self.button_add = wx.BitmapButton(
            self.toolbar, -1, wx.Bitmap('./image/button_plus.png'), pos=(20, 10), size=(16, 16))
        self.button_add.SetBitmapPressed(
            wx.Bitmap('./image/button_plus_on.png'))
        self.button_add.SetBitmapCurrent(
        ...
        self.button_add = wx.BitmapButton(
            self.toolbar, -1, wx.Bitmap('./image/button_folderplus.png'), pos=(55, 10), size=(16, 16))
        self.button_add.SetBitmapPressed(
            wx.Bitmap('./image/button_folderplus_on.png'))
        self.button_add.SetBitmapCurrent(
            wx.Bitmap('./image/button_folderplus_hover.png'))
        ...
        self.button_remove = wx.BitmapButton(
            self.toolbar, -1, wx.Bitmap('./image/button_cancel.png'), pos=(90, 10), size=(16, 16))
        self.button_remove.SetBitmapPressed(
            wx.Bitmap('./image/button_cancel_on.png'))
        self.button_remove.SetBitmapCurrent(
        ...
        wx.StaticText(
            self.toolbar, -1, 'copy to : <JE>./assets/minecraft/...    <BE>./...', pos=(135, 10))
        ...
        self.listbox = wx.ListBox(self.panel_bottom, -1, pos=(0, 0), size=(self.WIDTH-15, self.HEIGHT -
                                  self.TOOLBAR_HEIGHT-40), choices=bundle, style=wx.NO_BORDER | wx.VSCROLL | wx.HSCROLL)
    ...
    def click_add_file(self, event):
        dlg = wx.FileDialog(self, "リソースパックに同梱するファイルを選択",
                            style=wx.FD_OPEN | wx.FD_FILE_MUST_EXIST)
        if dlg.ShowModal() == wx.ID_OK:
            path = dlg.GetPath()
            filename = os.path.basename(path)
            if filename == 'sounds.json':
                Message().show(self, 'sounds.json は自動生成するため同梱不可です。ファイルを組み込む場合は手動で行ってください。')
            elif filename == 'sound_definitions.json':
                Message().show(self, 'sound_definitions.json は自動生成するため同梱不可です。ファイルを組み込む場合は手動で行ってください。')
